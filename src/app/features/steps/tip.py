...
def step_impl(ctx):
    user = User.objects.get(email="example@example.com")
    _create_website(user, 'Google', 'This is a link to google', 'http://www.google.fi')
    _create_website(user, 'Youtube', 'This is a link to youtube', 'http://www.youtube.com')


@when('the user enters a searchword "{searchword}"')
def step_impl(ctx, searchword):
    ctx.browser.fill("search", searchword)
